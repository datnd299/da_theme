 ...
 *
 * Override either value in wp-config.php if you rotate the keys later; the
 * defaults below are the keys configured for this site.
 */
...
/**
 * Load the Turnstile script on the contact page only.
 */
...
function dawp_handle_contact_form() {
    ...
    $support_email = function_exists('dawp_store_email') ? dawp_store_email() : get_option('admin_email');
    $store_name    = function_exists('dawp_store_name') ? dawp_store_name() : get_bloginfo('name');
    $subject       = sprintf('[%s] %s', $store_name, $topic_label);
    $body          = sprintf(
