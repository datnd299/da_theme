<?php
/**
 * Shop / Archive Product — YourWatchStore.
 * Structure only (HTML + PHP, no Tailwind). Styles live in assets/css/shop.css.
 */
...
$archive_description = __('Automatic dive, field, dress, and chronograph watches for everyday wear. Free US shipping and 30-day returns on every order.', 'dawp');
$archive_eyebrow = __('YourWatchStore Collection', 'dawp');
...
if ($archive_term && !is_wp_error($archive_term)) {
    ...
    $archive_eyebrow = is_product_tag() ? __('Shop By Tag', 'dawp') : __('Shop By Style', 'dawp');
}
    ...
    <div class="shop-header">
        <div class="shop-header__content">
            ...
            <div class="shop-header__meta">
                <span><?php printf(esc_html(_n('%d watch', '%d watches', $archive_total, 'dawp')), $archive_total); ?></span>
                <a href="<?php echo esc_url($shop_url); ?>"><?php esc_html_e('Shop all watches', 'dawp'); ?></a>
            </div>
        </div>
    </div>

    <?php if (!empty($category_chips)) : ?>
        <nav class="shop-category-nav" aria-label="<?php esc_attr_e('Watch styles', 'dawp'); ?>">
            ...
            <?php foreach ($category_chips as $chip) : ?>
                <a href="<?php echo esc_url($chip['url']); ?>" class="<?php echo $chip['current'] ? 'is-current' : ''; ?>" <?php echo $chip['current'] ? 'aria-current="page"' : ''; ?>>
    ...
    <div class="shop-toolbar">
        <div class="shop-toolbar__left">
            <span class="shop-toolbar__count">
                <?php printf('%d %s', $archive_total, $archive_total === 1 ? 'watch' : 'watches'); ?>
            </span>
    ...
    <div class="shop-layout">
        ...
        <aside class="shop-sidebar" id="shopSidebar" aria-label="Product filters">
            <div class="shop-sidebar__header">
                <h2 class="shop-sidebar__mobile-title">Filter Watches</h2>
                <button class="shop-sidebar__close" id="shopSidebarClose" aria-label="Close filters">
            ...
            <div class="shop-sidebar__widget">
                <h3 class="shop-sidebar__title">Styles</h3>
                <ul class="shop-sidebar__categories">
                    <li>
                        <a href="<?php echo esc_url($shop_url); ?>" <?php if (is_shop()) echo 'aria-current="page"'; ?>>All Watches</a>
                    </li>
                    <?php foreach ($categories as $cat) :
                        $is_current = is_product_category($cat->slug); ?>
                        <li>
                            <a href="<?php echo esc_url(get_term_link($cat)); ?>" <?php if ($is_current) echo 'aria-current="page"'; ?>>
        ...
        <main class="shop-main" id="main-content">
            ...
            <?php else : ?>
                <div class="shop-empty">
                    ...
                    <a href="<?php echo esc_url($shop_url); ?>">Browse all watches →</a>
                </div>
            <?php endif; ?>
