...
<body <?php body_class('bg-white text-[#1F2937] antialiased'); ?>>
<?php wp_body_open(); ?>
...
<header id="masthead" class="sticky top-0 z-50 border-b border-[#E8D9A6] bg-white text-[#1F2937] shadow-sm" role="banner">
        ...
        <div class="flex h-20 items-center justify-between gap-4">
            ...
            <nav class="hidden flex-1 items-center justify-center gap-5 lg:flex xl:gap-7" aria-label="<?php esc_attr_e('Primary navigation', 'dawp'); ?>">
                <?php foreach ($nav_items as $item) : ?>
                    <?php if (! empty($item['mega'])) : ?>
                        <div class="group/menu">
                            <a href="<?php echo esc_url($item['url']); ?>"
                               class="inline-flex h-20 items-center gap-1 whitespace-nowrap text-sm font-black uppercase tracking-wide text-[#1F2937] transition hover:text-[#C89B3C] focus:text-[#C89B3C] focus:outline-none">
                                        ...
                                        <div class="p-5">
                                            <div class="scott-mega-heading-row flex items-end justify-between gap-4 border-b border-[#E8D9A6]">
                                                <div>
                                                    ...
                                                    <h2 class="mt-2 font-heading text-xl font-black leading-tight text-[#1F6F68]">
                                                        <?php esc_html_e('Handmade and vintage-inspired finds', 'dawp'); ?>
                                                    </h2>
                                                </div>
                                                <a href="<?php echo esc_url($shop_url); ?>"
                                                   class="shrink-0 rounded-full border border-[#C89B3C] px-4 py-2 text-xs font-black uppercase tracking-wide text-[#1F6F68] transition hover:bg-[#F7F5EF]">
                                            ...
                                            <div class="mt-4 grid grid-cols-2 gap-3">
                                                <?php foreach ($shop_categories as $category) : ?>
                                                    <a href="<?php echo esc_url($category['url']); ?>"
                                                       class="group/category rounded-lg border border-[#E8D9A6] bg-[#F7F5EF] p-3 transition hover:border-[#C89B3C] hover:bg-white hover:shadow-sm">
                                                        <span class="flex items-center justify-between gap-3">
                                                            <span class="text-sm font-black text-[#1F6F68] group-hover/category:text-[#C89B3C]">
                                                                <?php echo esc_html($category['title']); ?>
                                                            </span>
                                                            <svg class="h-4 w-4 shrink-0 text-[#C89B3C]" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2.5" stroke-linecap="round" stroke-linejoin="round" aria-hidden="true">
                                                                <path d="M5 12h14"></path>
                                                                <path d="m12 5 7 7-7 7"></path>
                                                            </svg>
                                                        </span>
                                                        <span class="mt-2 block text-xs font-medium leading-5 text-[#475569]">
                                        ...
                                        <a href="<?php echo esc_url($featured_collection['url']); ?>"
                                           class="scott-mega-feature group/feature relative block overflow-hidden bg-[#1B4F49] text-white">
                                            ...
                                            <span class="scott-mega-feature-content relative flex h-full flex-col justify-end p-5">
                                                ...
                                                <span class="mt-3 text-sm font-medium leading-6 text-[#FAF6EA]">
                    ...
                    <?php else : ?>
                        <a href="<?php echo esc_url($item['url']); ?>"
                           class="whitespace-nowrap text-sm font-black uppercase tracking-wide text-[#1F2937] transition hover:text-[#C89B3C]">
                            <?php echo esc_html($item['title']); ?>
                        </a>
                    <?php endif; ?>
                <?php endforeach; ?>
            ...
            <div class="flex shrink-0 items-center gap-2 sm:gap-3">
                <form role="search"
                    ...
                    <input id="scott-header-search"
                           ...
                           class="h-10 w-48 rounded-full border border-[#E8D9A6] bg-[#F7F5EF] px-4 text-sm text-[#1F2937] placeholder:text-[#6E9B8E] outline-none transition focus:border-[#C89B3C] focus:bg-white focus:ring-2 focus:ring-[#C89B3C]/20">
                ...
                <a href="<?php echo esc_url($account_url); ?>"
                   class="hidden h-10 w-10 items-center justify-center rounded-full border border-[#E8D9A6] text-[#1F6F68] transition hover:bg-[#F7F5EF] sm:inline-flex"
                ...
                <a href="<?php echo esc_url($cart_url); ?>"
                   class="relative inline-flex min-h-10 items-center justify-center rounded-full bg-[#C89B3C] px-4 text-xs font-black uppercase tracking-wide text-white transition hover:bg-[#1F6F68] hover:text-white"
    ...
    <div id="scott-mobile-search" class="hidden border-t border-[#E8D9A6] bg-white xl:hidden">
        <form role="search"
            ...
            <input type="search"
                   ...
                   class="h-10 flex-1 rounded-full border border-[#E8D9A6] bg-[#F7F5EF] px-4 text-sm text-[#1F2937] placeholder:text-[#6E9B8E] outline-none focus:border-[#C89B3C] focus:bg-white focus:ring-2 focus:ring-[#C89B3C]/20">
            <input type="hidden" name="post_type" value="product">
            <button type="submit"
                    class="inline-flex h-10 w-10 shrink-0 items-center justify-center rounded-full bg-[#C89B3C] text-white transition hover:bg-[#1F6F68] hover:text-white"
    ...
    <div id="scott-mobile-menu" class="hidden border-t border-[#E8D9A6] bg-white lg:hidden">
        <nav class="mx-auto grid max-w-7xl gap-1 px-4 py-4 sm:px-6" aria-label="<?php esc_attr_e('Mobile navigation', 'dawp'); ?>">
            <?php foreach ($nav_items as $item) : ?>
                <a href="<?php echo esc_url($item['url']); ?>"
                   class="rounded-lg px-4 py-3 text-sm font-black uppercase tracking-wide text-[#1F6F68] transition hover:bg-[#F7F5EF] hover:text-[#C89B3C]">
                    <?php echo esc_html($item['title']); ?>
                </a>
                <?php if (! empty($item['mega'])) : ?>
                    <div class="grid gap-2 rounded-lg bg-[#F7F5EF] p-3">
                        <?php foreach ($shop_categories as $category) : ?>
                            <a href="<?php echo esc_url($category['url']); ?>"
                               class="rounded-lg bg-white px-4 py-3 text-sm font-bold text-[#1F6F68] transition hover:text-[#C89B3C]">
                                <?php echo esc_html($category['title']); ?>
                            </a>
                        <?php endforeach; ?>
                    </div>
