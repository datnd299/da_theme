<?php
/**
 * Side cart drawer — lightweight AJAX mini-cart for Corvel.
 *
 * Markup lives here, styles in assets/css/main.css (SIDE CART DRAWER block),
 * behaviour in assets/js/main.js (initCartDrawer).
 *
 * The header cart link (#dawp-cart-toggle in header.php) opens the drawer;
 * footer.php prints the drawer markup before wp_footer().
 *
 * @package dawp
 */
...
/**
 * Cart-count badge. Shared by the header link and the AJAX fragments so the
 * count stays in sync after every add / update / remove.
 */
function dawp_cart_count_badge_html($count) {
    $count = (int) $count;

    return sprintf(
        '<span class="dawp-cart-count%s">&nbsp;(%s)</span>',
        $count > 0 ? '' : ' is-empty',
        esc_html($count)
    );
}
...
function dawp_cart_drawer_footer() {
    ...
    <div class="dawp-cart-drawer__footer">
        <?php if ($cart && !$cart->is_empty()) : ?>
            ...
            <p class="dawp-cart-drawer__note"><?php esc_html_e('Free standard U.S. shipping. Taxes calculated at checkout.', 'dawp'); ?></p>
...
/**
 * Drawer shell. Printed once per page from footer.php.
 */
 ...
 * by WooCommerce core's own add_to_cart / remove_from_cart AJAX endpoints so
 * the drawer stays in sync no matter which flow triggered the change.
 */
...
function dawp_cart_fragments($fragments) {
    ...
    $fragments['.dawp-cart-count']          = dawp_cart_count_badge_html($count);
...
function dawp_cart_ajax_add() {
    ...
    // Buffer and discard stray output (e.g. a notice from a hooked plugin) so
    // it can never leak into the JSON response.
    ob_start();
