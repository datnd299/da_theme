 ...
 *
 * Shipping policy content adapted for Shop Kelli.
 */
...
<style>
  .sk-ship-page { --sk-cream:#FAF7F2; --sk-surface:#F5F3F1; --sk-rose:#c98a8a; --sk-rose-dark:#b87575; --sk-beige:#E8D8C8; --sk-blush:#DFA39A; --sk-ink:#2F2A28; --sk-muted:#6F625D; --sk-soft:#9A8C86; --sk-border:#E6DDD6; background:#fff; color:var(--sk-muted); font-family:"Lato","Inter",system-ui,sans-serif; }
  ...
  .sk-ship-title { margin:0; color:var(--sk-ink); font-family:"Playfair Display",Georgia,"Times New Roman",serif; font-size:clamp(36px,5vw,64px); line-height:1.04; letter-spacing:0; }
  ...
  .sk-ship-hero { position:relative; overflow:hidden; background:linear-gradient(135deg,rgba(250,247,242,.96),rgba(232,216,200,.62) 46%,rgba(223,163,154,.34)),#fff; }
  .sk-ship-hero::before { content:""; position:absolute; inset:24px auto auto 8%; width:220px; height:220px; border-radius:999px; background:rgba(255,255,255,.52); filter:blur(8px); }
  .sk-ship-hero::after { content:""; position:absolute; right:7%; bottom:-92px; width:360px; height:360px; border:1px solid rgba(201,138,138,.22); border-radius:999px; background:rgba(255,255,255,.22); }
  ...
  .sk-ship-policy-card, .sk-ship-contact-card { border:1px solid var(--sk-border); border-radius:20px; background:rgba(255,255,255,.94); box-shadow:0 18px 46px rgba(47,42,40,.08); }
  ...
  .sk-ship-policy-card { padding:clamp(24px,4vw,38px); background:#fff; }
  .sk-ship-policy-card:nth-child(even) { background:#fffafa; }
  .sk-ship-policy-card h2 { margin:0; color:var(--sk-ink); font-family:"Playfair Display",Georgia,"Times New Roman",serif; font-size:clamp(25px,3vw,38px); line-height:1.12; letter-spacing:0; }
  ...
  .sk-ship-callout { border-left:4px solid var(--sk-rose); border-radius:0 16px 16px 0; background:#fff7ed; padding:15px 18px; }
  .sk-ship-split-grid { display:grid; grid-template-columns:repeat(2,minmax(0,1fr)); gap:16px; margin-top:18px; }
  .sk-ship-info-panel { border:1px solid var(--sk-border); border-radius:16px; background:#fff; padding:18px; }
  .sk-ship-info-panel--soft { background:#fffafa; }
  ...
  .sk-ship-timeline-item { border:1px solid var(--sk-border); border-radius:16px; background:#fff; padding:18px; }
  ...
  .sk-ship-policy-card .sk-ship-carrier-list li { display:inline-flex; width:auto; min-width:72px; align-items:center; justify-content:center; border:1px solid var(--sk-border); border-radius:999px; background:#fff; padding:9px 16px; color:var(--sk-ink); font-size:13px; font-weight:800; line-height:1.2; }
  ...
  .sk-ship-contact-item { border:1px solid var(--sk-border); border-radius:14px; background:#fff; padding:16px; }
  ...
  <section class="sk-ship-hero">
    <div class="sk-ship-wrap sk-ship-hero__grid">
      <div class="sk-ship-hero__content">
        ...
        <p class="sk-ship-copy"><?php esc_html_e('Shop Kelli currently ships exclusively within the United States, with free standard U.S. shipping for every order and clear delivery timelines shown before checkout is completed.', 'dawp'); ?></p>
    ...
    <div class="sk-ship-wrap sk-ship-content-grid">
      <div class="sk-ship-policy-stack">
        <section id="shipping-locations" class="sk-ship-policy-card">
          ...
          <p><?php esc_html_e('We currently ship exclusively within the United States. Shop Kelli serves customers shopping from the United States domestic market.', 'dawp'); ?></p>
          ...
          <div class="sk-ship-callout">
            <p><?php esc_html_e('Some boutique clothing orders may ship separately if items are prepared from different fulfillment batches or require distinct packing methods to keep garments, accessories, and seasonal pieces protected in transit.', 'dawp'); ?></p>
          </div>
        </section>
        ...
        <section id="multi-item-orders" class="sk-ship-policy-card">
          ...
          <p><?php esc_html_e('If your purchase includes multiple boutique clothing pieces, accessories, mommy & me styles, or girls collection items, they may be fulfilled from different locations. Consequently, your items may ship separately and arrive in multiple packages.', 'dawp'); ?></p>
          <p><?php esc_html_e('You will receive unique tracking numbers for each package. Certain high-demand seasonal pieces, matching sets, or carefully packed boutique items may require extra preparation time due to rigorous address reviews, holiday volume spikes, or safe-handling protocols.', 'dawp'); ?></p>
        </section>

        <section id="tracking" class="sk-ship-policy-card">
          <h2><?php esc_html_e('Carrier Services & Delivery Tracking', 'dawp'); ?></h2>
          <p><?php esc_html_e('To guarantee safe and efficient delivery, Shop Kelli partners with trusted domestic U.S. carriers. Orders are shipped using USPS, UPS, FedEx, or DHL.', 'dawp'); ?></p>
        ...
        <section id="delivery-issues" class="sk-ship-policy-card">
          ...
          <ul>
            ...
            <li><?php esc_html_e('Clear, well-lit photos if the package container or boutique item arrived damaged.', 'dawp'); ?></li>
          </ul>
          <div class="sk-ship-actions">
            <a class="sk-ship-button" href="<?php echo esc_url($support_portal); ?>"><?php esc_html_e('Contact Support', 'dawp'); ?></a>
