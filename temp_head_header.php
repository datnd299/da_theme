...
<header id="site-header" class="sticky top-0 z-50 border-b border-[#E5E7EB] bg-[#FFFFFF]/95 text-[#1F2937] backdrop-blur" role="banner">
    <div class="hidden border-b border-[#E5E7EB] bg-[#F5F6F8] lg:block">
        <div class="mx-auto flex max-w-7xl items-center justify-between gap-4 px-8 py-2 text-xs font-semibold text-[#6B7280]">
            <p><?php esc_html_e('Collectible toys, building sets, figures, and display pieces for modern collectors.', 'dawp'); ?></p>
    ...
    <div class="mx-auto max-w-7xl px-4 sm:px-6 lg:px-8">
        <div class="flex min-h-20 items-center justify-between gap-4">
            <a href="<?php echo esc_url($home_url); ?>" class="inline-flex shrink-0 items-center py-2" aria-label="<?php esc_attr_e('BrickGo home', 'dawp'); ?>">
                <?php
                echo function_exists('dawp_get_responsive_image')
                    ? dawp_get_responsive_image($logo_url, __('BrickGo', 'dawp'), 'site-logo-img', 128, 128, 'eager', '(max-width: 520px) 104px, 128px', 'high')
                    : '<img class="site-logo-img" src="' . esc_url($logo_url) . '" width="1290" height="1290" alt="' . esc_attr__('BrickGo', 'dawp') . '" decoding="async" fetchpriority="high">';
                ?>
            </a>
            ...
            <div class="flex shrink-0 items-center gap-2">
                <form role="search" method="get" action="<?php echo esc_url(home_url('/')); ?>" class="hidden items-center rounded-md border border-[#E5E7EB] bg-white px-3 py-2 lg:flex">
                    ...
                    <input id="header-product-search" type="search" name="s" value="<?php echo esc_attr(get_search_query()); ?>" placeholder="<?php esc_attr_e('Search figures, builds, blind boxes', 'dawp'); ?>" class="w-40 bg-transparent text-sm text-[#1F2937] outline-none placeholder:text-[#8D7D77]">
    ...
    <div id="mobile-store-menu" class="hidden border-t border-[#E5E7EB] bg-[#FFFFFF] xl:hidden">
        <div class="mx-auto max-w-7xl px-4 py-4 sm:px-6">
            <form role="search" method="get" action="<?php echo esc_url(home_url('/')); ?>" class="mb-4 flex items-center rounded-md border border-[#E5E7EB] bg-[#F5F6F8] px-4 py-3">
                ...
                <input id="mobile-product-search" type="search" name="s" value="<?php echo esc_attr(get_search_query()); ?>" placeholder="<?php esc_attr_e('Search figures, builds, blind boxes', 'dawp'); ?>" class="w-full bg-transparent text-sm text-[#1F2937] outline-none placeholder:text-[#8D7D77]">
                <input type="hidden" name="post_type" value="product">
